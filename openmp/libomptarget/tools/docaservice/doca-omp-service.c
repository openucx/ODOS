...
{
	DOCA_LOG_INFO("[EXP] CMD_DATA_DELETE(%d) 0x%09x", cmd->cmd, cmd->data.ptr);
...
int process_cmd_load_binary(struct bf_openmp_cmd *cmd)
{
	int i, ret;
	size_t msglen;
	...
	FILE *ftmp = NULL;
	int tmp_fd = -1;
	char tmp_name[128] = "/var/tmp/omp_tmpfile_XXXXXX";
...
int process_cmd_run_target_region(struct bf_openmp_cmd *cmd)
{
	...
	if (cmd->run.total_args) {
		bf_recv_buf(conn, args, cmd->run.total_args * sizeof(void *   ));
		bf_recv_buf(conn, offs, cmd->run.total_args * sizeof(ptrdiff_t));
	}
	...
	ffi_status Status = ffi_prep_cif(
		&cif, FFI_DEFAULT_ABI, cmd->run.total_args, &ffi_type_void, &ArgsTypes[0]);
	if (Status != FFI_OK) {
...
int run_service()
{
	...
	while (1) {
		...
		DOCA_LOG_DBG("cmd: %d\n", cmd.cmd);
		switch(cmd.cmd) {
			case CMD_LOAD_BINARY:
				ret = process_cmd_load_binary(&cmd);
				break;
			case CMD_DATA_ALLOC:
